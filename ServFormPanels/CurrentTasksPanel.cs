...
namespace CarDealerSupportSystem.SellerFormPanels
{
    public partial class CurrentTasksPanel : Form
    {
        ...
        public void LoadCurrentTasks()
        {
            using (salon_samochodowyContext db = new())
            {
                var currentTasks = (from z in db.Zamowienia
                                    ...
                                    where sz.IdPracownika == this.servId && sz.Status == "wtrakcie"  //==this.id
        ...
        private void TasksGridView_CellClick(object sender, DataGridViewCellEventArgs e)//info o wykonywanym zadaniu
        {
            if (e.RowIndex >= 0)
            {
                string[] taskInfo;
                using (salon_samochodowyContext db = new())
                {
                    ...
                    var taskMoreInfo = (from z in db.Zamowienia
                                        ...
                                        join u in db.Uslugi on sz.IdUslugi equals u.IdUslugi
                                        where z.IdZamowienia == _taskId && sz.IdPracownika==this.servId
